    ...
    <div className='relative py-4 px-6 md:px-8 bg-white'>
        <div className='flex flex-col md:flow-row items-start gap-6'>
            <div className='w-32 h-32 border-4 border-white shadow-lg absolute -top-16 rounded-full'>
                <img src={user.profile_picture} alt='Profile' className='absolute z-2 rounded-full' />
            </div>
            ...
            <div className='w-full pt-16 md:pt-0 md:pl-36'>
                <div className='flex flex-col md:flex-row  items-start justify-between'>
                    ...
                    {!profileId && (
                        <button onClick={() => setShowEdit(true)} className='flex items-center gap-2 border border-gray-300 cursor-pointer hover:bg-gray-50 px-4 py-2 rounded-lg font-medium transition:colors mt-4 md:mt-0'>
                            <PenBox className='w-4 h-4' />
                            Edit Profile
                        </button>
                    )}
                </div>
                <p className='text-gray-700 text-sm max-w-md mt-4'>{user.bio ? user.bio : 'Add a bio'}</p>
                ...
                <div className='flex items-center gap-6 mt-6 border-t border-gray-200 py-4'>
                    <div>
